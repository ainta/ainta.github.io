...
author: ainta
tags: [algorithm, matroid, greedy, structure, graph-theory]
...
$\mathcal{I_1} = \left\lbrace I : I \subset E, V_1의 \: 각 \: 정점은 \: I에 \: 포함되는 \: 간선 \: 최대 \: 하나의 \: 끝점  \right\rbrace$으로 두면  $\mathcal{M_1} = (V_1, \mathcal{I})$ 은 matroid이다.

$\mathcal{I_2} = \left\lbrace I : I \subset E, V_2의 \: 각 \: 정점은 \: I에 \: 포함되는 \: 간선 \: 최대 \: 하나의 \: 끝점  \right\rbrace$으로 두면  $\mathcal{M_2} = (V_2, \mathcal{I})$ 은 matroid이다.
...
**정의 1.**  matroid $\mathcal{M} = (S, \mathcal{I})$에서 independent set $I$에 대해 directed bipartite graph $D_M(I)$를 다음과 같이 정의하자: $D_M(I) = (S, E)$, $ E = \left\lbrace (y,z) \in E \: : \:  y \in I, z \in S \setminus I , I-y+z \in \mathcal{I}  \right\rbrace$. $D_M(I)$의 bipartition은 $(I, S \setminus I)$이다.
...
**Proof:**  그래프 $D_M(I)$에서 $I \Delta J$에 포함되는 vertex들과 그 vertex 사이의 간선만 남긴 그래프를 $G$라 하자 ( $G$ : subgraph of $D_M(I)$ induced by $I \Delta J$ ). $G$는 bipartite graph 이므로 [성질 5]에 의해 $I \setminus J$의 vertex들을 $y_1,.., y_t$, $J \setminus I$의 vertex들을 $z_1, .., z_t$로 라벨링하여 $N = \left\lbrace (y_1, z_1), .., (y_t, z_t) \right\rbrace$이고 $ \forall  1 \le i < j \le t $,  $(y_i, z_j) \notin E(G)$  가 성립하도록 할 수 있다.
...
**정의 6.** $D_{M_1, M_2}(I) = (S, A(I))$ where $A(i) = \left\lbrace (y, z) : y \in I, z \in S \setminus I, I-y+z \in \mathcal{I_1} \right\rbrace \cup \left\lbrace (z', y') : y' \in I, z' \in S \setminus I, I-y'+z' \in \mathcal{I_2} \right\rbrace$. 다르게 말하면, $D_{M_1, M_2}(I)$는 $D_{M_1}(I)$와 $D_{M_2}(I)$의 reverse의 union이다. 

$X_1 = \left\lbrace z \in S \setminus I : I + z \in \mathcal{I_1}\right\rbrace$, $X_2 = \left\lbrace z \in S \setminus I : I + z \in \mathcal{I_2}\right\rbrace$ 라 하자. $D_{M_1,M_2}(I)$에서 $X_1$에서 출발해 $X_2$에서 끝나는 경로 중 최소 길이인 것을 $P$라 하자. ($P$는 존재하지 않을 수도 있다)
...
**Proof:** $P$는 bipartite graph  $D_{M_1,M_2}(I) $에서  $S \setminus I$ 로부터 시작해  $S \setminus I$로 끝나는 경로이므로 $V(P)$에서 $I$에 포함되는 원소보다 $S \setminus I$에 포함되는 원소가 항상 하나 더 많다. 따라서 $\lvert I' \rvert = \lvert I \Delta V(P) \rvert = \lvert I \rvert+1$이 성립한다. $P = z_0, y_1, z_1, ..., z_{t-1}, y_t, z_t$라 두자($z_i \in S \setminus I, y_i \in I$). 집합  $J = \left\lbrace z_1, ..., z_t \right\rbrace \cup (I \setminus \left\lbrace y_1, .., y_t\right\rbrace)$ 에 대해 $J \subset S$, $\lvert J \rvert = \lvert I \rvert$이고 $\left\lbrace y_1, ..., y_t \right\rbrace 에서 \left\lbrace z_1, ..., z_t \right\rbrace$로 가는 간선들은 $I \setminus J$에서 $J \setminus I$로 가는 unique perfect matching을 이룬다 (matching이 존재함은 자명하고, unique하지 않다면 [성질 5]에 의해 $i < j$인 간선 $(y_i, z_j)$가 존재하는데 이는 $P$가 shortest path임에 모순이다). 따라서, 정리 4에 의해 $J \in \mathcal{I_1}$.
...
Step 2. $D_{M_1,M_2}$를 만들고, 집합 $X_1 = \left\lbrace z \in S \setminus I : I + z \in \mathcal{I_1}\right\rbrace$, $X_2 = \left\lbrace z \in S \setminus I : I + z \in \mathcal{I_2}\right\rbrace$ 를 구하자.
...
이전 글 Introduction to matroid에서 graphic matroid를 소개할 때, edge들이 forest를 이루도록 하는 일반적인 graphic matroid만 설명한 것이 아니라 변형된 몇 가지 graphic matroic를 소개하였다. 그 중 하나가 바로 이 문제에 사용되는 매트로이드인데,  $G = (V, E)$가 무향 연결 그래프일 때, $\mathcal{I} = \left\lbrace I : I \subset E, E - I \: connects \: all \: vertex \: in \: G \right\rbrace$로 두면 $\mathcal{M} = (E, \mathcal{I})$는 matroid이다. 이 문제를 풀기 위해 이를 살짝 변형해보자. $G = (V, E)$가 무향 연결 그래프일 때, $\mathcal{I_1} = \left\lbrace I : I \subset E, E - I \: 중 \: 색깔이 \: R\: 또는 \: G \:인 \:것만 \: 고려해도 \:G는 \:connected \right\rbrace$로 두면 $\mathcal{M_1} = (E, \mathcal{I_1})$는 matroid이다.  $\mathcal{I_2} = \left\lbrace I : I \subset E, E - I \: 중 \: 색깔이 \: G\: 또는 \: B \:인 \:것만 \: 고려해도 \:G는 \:connected \right\rbrace$로 두면 $\mathcal{M_2} = (E, \mathcal{I_1})$는 matroid이다. 
