...
이번 포스트에서는 전 글에서 subquadratic approximation algorithm의 결과를 얻었다고 소개했던 [Faster Matroid Intersection(2019)](https://arxiv.org/pdf/1911.10765.pdf)에 대해 다룬다. 기존 알고리즘은 exchange graph $G(S)$에서 augmenting path를 하나씩 찾아 $S$를 업데이트하지만, 이 논문은 여러 path를 한 번에 묶은 구조인 augmenting set을 도입하고, partial augmenting set의 연속된 Refine으로 이를 얻어낸 뒤 Hybrid 알고리즘으로 결합해 $\tilde{O}((n/\epsilon)^{1.5})$ oracle call 만에 $1-\epsilon$ approximation을 달성한다.


## Preliminaries

전 글과 동일하게, matroid $\mathcal{M_1} = (V, \mathcal{I_1}), \mathcal{M_2} = (V, \mathcal{I_2})$가 주어진 세팅이다. 이후 증명들에서 반복적으로 쓰는 세 가지 도구 — submodularity of rank, Monotonicity Lemma, Fact 1 — 을 먼저 정리해둔다. 각각은 augmenting set의 정당성과 Refine subroutine의 불변량 분석에 쓰인다.


**Submodularity of rank.** $A, B \subset V$에 대해, $rank(A \cup B) + rank(A \cap B) \le rank(A) + rank(B)$가 성립한다.

**Proof.** $rank(A \cup B) - rank(A) \le rank(B) - rank(A \cap B)$를 보이면 충분하다. 이는 $rank(A + (B \setminus A)) - rank(A) \le rank(A\cap B + (B \setminus A)) - rank(A \cap B)$로 쓸  수 있고,
...
**Proof.** $rank(S-X+Y+Z) + rank(S+Y) \ge rank(S-X+Y) + rank(S+Y+Z)$ (Submodularity of rank). $rank(S+Y) = \lvert S \rvert, rank(S-X+Y) =  \lvert S \rvert - \lvert X \rvert + \lvert Y \rvert , rank(S+Y+Z) \ge rank(S+Z) =  \lvert S \rvert + \lvert Z \rvert$.
...
shortest augmenting path를 일반화한 개념인 augmenting set을 정의하고, (1) augmenting set으로 $S$를 업데이트하면 여전히 common independent set이며, (2) maximal augmenting set의 width가 0이면 $S$가 답임을 보인다. 이로써 augmenting set을 찾아 업데이트하는 것을 반복하는 알고리즘의 뼈대가 잡힌다.

$S \in \mathcal{I_1} \cap \mathcal{I_2}$의 corresponding exchange graph $G(S)$에 대해, $G(S)$의 shortest $(s,t)$-path의 길이가 $2(l+1)$이라 하자.


**Definition(Distance Layer).** $G(S)$의 Distance layer $D_1, D_2, \cdots, D_{2l+1}$ 은 $D_i :=$ ($G(S)$에서 $s$로부터의 최단거리가 $i$인 원소들의 집합) 으로 정의된다.

**Definition(Augmenting Set).** set들의 collection $\Pi_l := (B_1, A_1, \cdots , A_l, B_{l+1})$가 다음 조건을 만족할 때 $\Pi_l$을 Augmenting Set이라 하고, 공통된 크기 $w$를 width라 부른다:

- (a) For $1 \le k \le l+1$, we have $A_k \subset D_{2k}$ and $B_k \subset D_{2k-1}$  — 각 $A_k, B_k$가 올바른 distance layer에 놓여있다
- (b) $\lvert B_1 \rvert  =  \lvert A_1 \rvert  = \cdots  \lvert B_{l+1} \rvert  = w$  — 모든 layer에서 같은 width $w$ (path 대응 관점에서는 path 개수)
- (c) $S + B_1 \in \mathcal{I_1}, S + B_{l+1} \in \mathcal{I_2}$  — source 쪽은 $\mathcal{M_1}$에, sink 쪽은 $\mathcal{M_2}$에 독립
- (d) $S - A_k + B_{k+1} \in \mathcal{I_1}$  — $\mathcal{M_1}$에서 각 layer 간 exchange가 가능
- (e) $S - A_k + B_{k} \in \mathcal{I_2}$  — $\mathcal{M_2}$에서 각 layer 간 exchange가 가능


**Theorem.** $S' = S \oplus \Pi_l = S + B_1 - A_1 + B_2 -  \cdots + B_l - A_l + B_{l+1}$ 은 common independent set. 즉, $S' \in \mathcal{I_1} \cap \mathcal{I_2}$.

**Proof.** Fact 1을 반복 적용한다. $S+B_1 \in \mathcal{I_1}, S - A_1 + B_2 \in \mathcal{I_1}, rank_1(S + B_2) = rank_1(S)$ 이므로 ($\mathcal{M_1}$ 에서 $S$와 독립인 원소들은 $D_1$에 있으므로 $B_2, \cdots B_{l+1}$엔 들어갈 수 없다) $S + B_1 - A_1 + B_2 \in \mathcal{I_1}$이고, 이를 반복하면 $S' \in \mathcal{I_1}$.

한편, $S + B_{l+1} \in \mathcal{I_2}, S - A_l + B_l \in \mathcal{I_2}, rank_2(S+B_l) = rank_2(S)$ 이므로 (shortest path가 $2(l+1)$이므로 $\mathcal{M_2}$에서 $S$와 독립인 원소들은 $B_{l+1}$에만 들어갈 수 있다)  $S + B_{l+1} - A_l + B_l \in \mathcal{I_2}$이고, 이를 반복하면 $S' \in \mathcal{I_2}$. $\blacksquare$


**Theorem.** Maximal augmenting set의 width가 0인 $S$는 $\mathcal{I_1} \cap \mathcal{I_2}$의 maximum cardinality set이다.

핵심 아이디어는 width가 0이면 $X_1 \to X_2$ augmenting path가 없다는 것으로, $X_2$에 도달 가능한 정점들의 집합을 $U$로 두면 matroid duality $\lvert S \rvert = rank_1(U) + rank_2(V \setminus U)$ 가 성립해 $S$의 크기가 matroid intersection 상한을 포화시킨다는 점이다.

**Proof.** $S+v \in \mathcal{I_1}$인 $v$들의 집합을 $X_1$, $S+v \in \mathcal{I_2}$인 $v$들의 집합을 $X_2$라 하자. Maximal augmenting set의 width가 0이므로, $G(S)$는 augmenting path가 존재하지 않는다. 즉, $X_1$에서 $X_2$로 가는 path가 존재하지 않는다. $X_1$이나 $X_2$가 빈 집합이라면 $S$는 두 매트로이드 중 하나의 base이므로 maximum cardinality set임이 자명하다. $X_1 \neq \phi, X_2 \neq \phi$라 하자. $U$를 $X_2$에 도달할 수 있는 vertex들의 집합이라 하면 $X_1$에서 $X_2$로 가는 path가 없으므로 $X_1 \cap U = \phi$이다. 먼저, $rank_1(U) \le \lvert S \cap U \rvert$, $rank_2(V \setminus U) \le \lvert S \setminus U \rvert$ 임을 보이자.
...
따라서, $\lvert S' \rvert  \le rank_1(U) + rank_2(V \setminus U) \le  \lvert S \rvert$ 이고 $S$는 $\mathcal{I_1} \cap \mathcal{I_2}$의 maximum cardinality set이다. $\blacksquare$
...
width $w$의 augmenting set은 $w$개의 disjoint shortest $(s,t)$-path들이 **순서대로** consecutive shortest paths를 이루는 것과 동치임을 보인다. 이 덕분에 augmenting set 관점과 path 관점을 자유롭게 오가며 분석할 수 있다.
...
**Proof.** $G(S \oplus \Pi)$에 길이 $2(l+1)$의 augmenting path $p$가 존재한다고 가정하자. Monotonicity Lemma에 의해, $p$는 $G(S)$에서도 augmenting path여야 한다.
...
Augmenting set의 제약 (b) — 모든 layer에서 width가 같다 — 를 완화한 partial augmenting set을 도입한다. width가 단조 감소만 하도록 허용되어 중간 상태로 쓸 수 있고, 이로부터 augmenting set을 재구성할 수 있다 (Lemma 5). 또한 서로 다른 maximal augmenting set들의 width 차이가 $(2l+4)$배 이내임도 증명한다 (Lemma 6). 두 Lemma는 이후 Hybrid 알고리즘의 정당성의 핵심이 된다.

앞서 Augmenting Set을 구하여 $S$를 업데이트하는 것을 반복하여 Matroid Intersection Problem을 해결할 수 있음을 보였고, 특히 Maximal Augmenting Set을 구하여 $S$를 업데이트한다면 $G(S)$의 shortest $(s,t)$-path가 증가하므로 $O(n)$번의 업데이트로 답을 구할 수 있음을 보였다.

그러나, Maximal Augmenting Set을 적은 oracle로 구하는 것은 쉽지 않았고, 이에 Partial Augmenting Set의 개념이 도입되게 되었다.

**Definition(Partial Augmenting Set).** set들의 collection $\Psi_l := (B_1, A_1, \cdots , A_l, B_{l+1})$가 다음 조건을 만족할 때 $\Psi_l$을 Partial Augmenting Set이라 한다:

- (a) For $1 \le k \le l+1$, we have $A_k \subset D_{2k}$ and $B_k \subset D_{2k-1}$  — augmenting set과 동일
- (b) $\lvert B_1 \rvert  \ge  \lvert A_1 \rvert \ge \cdots \ge  \lvert B_{l+1} \rvert$  — 핵심 완화: width가 모두 같을 필요 없이 단조 감소만 하면 된다
- (c) $S + B_1 \in \mathcal{I_1}, S + B_{l+1} \in \mathcal{I_2}$  — 동일
- (d) $S - A_k + B_{k+1} \in \mathcal{I_1}$  — 동일
- (e) for $k \le l, rank_2(S - A_k + B_{k}) = rank_2(S)$  — augmenting set에서는 $\in \mathcal{I_2}$였는데 여기서는 full rank 유지 조건으로 완화
...
**Proof.** $\Pi'$를 $B_{l+1}, A_l, \cdots B_1$의 **순서대로** 구성한다. 처음에 $\Pi'$의 $B_{l+1}$을  $\Psi$의 $B_{l+1}$과 동일하게 둔다.
...
**Proof.** $\Pi = (B_1, A_1, \cdots B_{l+1})$, $\Psi = (Q_1, P_1, \cdots Q_{l+1})$ 가 각각 maximal augmenting set이라 하자.
...
이제 $S$가 있을 때 $G(S)$의 maximal augmenting set을 찾는 방법에 대해 알아보자. partial augmenting set을 점진적으로 키워 maximal로 만드는 Refine subroutine을 설계하고, 세 가지 불변량을 선언한 뒤 Refine1/Refine2 각 스텝이 이를 보존함을 보인다. 이로부터 총 oracle 횟수 $O(n^3)$을 유도하고, 이어서 approximation 버전이 왜 $O(n^2/\epsilon)$으로 줄어드는지 설명한다.
...
**Proof.** Refine1($k$)에서는 $O(\lvert D_{2k} + D_{2k+1} \rvert)$번의 oracle call을 사용하고, Refine2($k$)에서는 $O(\lvert D_{2k-1} + D_{2k} \rvert)$번의 oracle call을 사용하므로 한 번의 Refine subroutine에서는 $O(n)$번의 independent oracle call을 사용한다.


**Theorem.** Refine을 아무 element도 이동하지 않을 때까지 하면 maximal augmenting set이 된다.

**Proof.** Refine했는데 아무 element도 이동하지 않는다면 $\lvert B_1 \rvert  =  \lvert A_1 \rvert  = \cdots =  \lvert A_l \rvert  =  \lvert B_{l+1} \rvert$이므로 augmenting set. 이제 maximality만 보이면 된다.
...
따라서, $b_1, a_1, \cdots, b_{l+1}$ 는 모두 removed 상태. 그런데 $b_{l+1}$이 removed 되는 과정은 refine에 없고, 따라서 모순. $\blacksquare$


**Lemma.** Maximal augmenting set을 얻을 때 까지 수행해야 하는 Refine subroutine의 횟수는 $O(n)$이다.

**Proof.** 모든 과정을 통틀어 각 element의 상태는 최대 2번 바뀐다. ($fresh \rightarrow selected \rightarrow removed$). 따라서, $O(n)$번 이하의 refine으로 Maximal augmenting set을 얻는다.$\blacksquare$
...
**Proof.** $\Phi_{l}$에서 Refine 과정을 update가 일어나지 않을 때까지 반복하여 maximal augmenting set $\Pi'_l$를 얻는 과정동안 $\Pi_l$의 원소들이 removed되지 않도록 수행하는 방법을 제시할 것이다.
...
**Proof.** 위 Lemma로부터 $\lvert \Pi'_l - \Pi_l \rvert \le p$인 $G(S)$의 maximal augmenting set $\Pi'_l$가 존재하여 $\Pi_l$를 포함한다.
