...
더 나아가, $X$의 어떤 subset들의 집합 $A = \left\lbracea_1, a_2, .., a_n \right\rbrace $에 대해, $A$의 원소들을 모두 open set으로 가지는 topology의 개수를 효율적으로 구할 수 있을까?
...
그렇다면, 다음 문제를 해결하는 것으로 충분하다 : $X = \left\lbrace 0, 1, ... , k-1 \right\rbrace$의 어떤 subset들의 집합 $A = \left\lbracea_1, a_2, .., a_n \right\rbrace $에 대해, $A$의 원소들을 모두 open set으로 가지는 topology의 개수를 효율적으로 구할 수 있을까?
...
$\left\lbraceT_1, T_2, T_3, T_4, T_5, T_6, T_7\right\rbrace$ = $ \left\lbrace1, 4, 29, 355, 6942, 209527, 9535241\right\rbrace$ 이고  $\sum_{i=1}^k T_i \times i \le 7 \times 10^7$이므로, 빠른 시간 (1초) 이내에 조건을 만족하는 모든 good set을 탐색할 수 있다.
