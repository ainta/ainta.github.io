...
더 나아가, $X$의 어떤 subset들의 집합 $A = \left\lbrace a_1, a_2, .., a_n \right\rbrace $에 대해, $A$의 원소들을 모두 open set으로 가지는 topology의 개수를 효율적으로 구할 수 있을까?
...
topological space의 정의에서, 만약 $X$가 유한 개의 원소로 이루어져 있다면, 2번 조건 및 3번 조건을 각각 임의의 원소들의 합집합, 유한개 원소들의 교집합이 아닌 단 두 개의 원소의 합집합/교집합으로 바꿔도 같은 정의이다. 따라서, 문제에서 구하는 good set의 정의는 $X = \left\lbrace 0, 1, ... , k-1 \right\rbrace$ 일 때, topology on $X$의 정의와 거의 일치한다. 공집합과 전체집합을 포함한다는 조건이 빠져 있는데, 이는 쉽게 처리할 수 있다.

그렇다면, 다음 문제를 해결하는 것으로 충분하다 : $X = \left\lbrace 0, 1, ... , k-1 \right\rbrace$의 어떤 subset들의 집합 $A = \left\lbrace a_1, a_2, .., a_n \right\rbrace $에 대해, $A$의 원소들을 모두 open set으로 가지는 topology의 개수를 효율적으로 구할 수 있을까?
...
$X = \left\lbrace 0, 1, ... , k-1 \right\rbrace$ 에서의 Topology $T$에 대해, 그래프 $f(T) = (V, E)$ 를 다음과 같이 정의하자 : 서로 다른 ​$u, v$에 대해, ​$T$의 ​$u$를 원소로 가지는 모든 open set들이 ​$v$ 역시 원소로 가질 때만 ​$(u, v) \in E$.

이 때, $f(T)$가 transitive digraph인 것은 정의에 의해 자명하다. 

$V = \left\lbrace 0, 1, ... , k-1 \right\rbrace$인 transitive digraph $G = (V, E)$에 대해, $V$의 부분집합들의 집합 $g(G)$를 다음과 같이 정의하자. 
...
$\left\lbrace T_1, T_2, T_3, T_4, T_5, T_6, T_7\right\rbrace$ = $ \left\lbrace1, 4, 29, 355, 6942, 209527, 9535241\right\rbrace$ 이고  $\sum_{i=1}^k T_i \times i \le 7 \times 10^7$이므로, 빠른 시간 (1초) 이내에 조건을 만족하는 모든 good set을 탐색할 수 있다.
